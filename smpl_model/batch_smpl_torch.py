...
class SMPLModel(Module):
    def __init__(self, device=None, model_path='./model.pkl', use_posematrix=False):

        super(SMPLModel, self).__init__()
        with open(model_path, 'rb') as f:
            params = pickle.load(f, encoding='iso-8859-1')
        self.J_regressor = torch.from_numpy(np.array(params['J_regressor'].todense())
                                            if type(params['J_regressor']) is not np.ndarray else params['J_regressor']).type(torch.float32)
        # if 'joint_regressor' in params.keys():
        #     self.joint_regressor = torch.from_numpy(np.array(params['joint_regressor'].T.todense())).type(torch.float32)
        # else:
        #     self.joint_regressor = torch.from_numpy(np.array(params['J_regressor'].todense())).type(torch.float32)
        self.weights = torch.from_numpy(np.array(params['weights'])).type(torch.float32)
        self.posedirs = torch.from_numpy(np.array(params['posedirs'])).type(torch.float32)
        self.v_template = torch.from_numpy(np.array(params['v_template'])).type(torch.float32)
        self.shapedirs = torch.from_numpy(np.array(params['shapedirs'])).type(torch.float32)
    ...
    def with_zeros(x):
        ...
        """
        ones = torch.tensor([[[0.0, 0.0, 0.0, 1.0]]], dtype=torch.float32 ).expand(x.shape[0], -1, -1).to(x.device)
    ...
    def pack(x):
        ...
        """
        zeros43 = torch.zeros( (x.shape[0], x.shape[1], 4, 3), dtype=torch.float32).to(x.device)
    ...
    def forward(self, betas, pose, trans, displacements, simplify=False):
        ...
        if simplify:
            ...
            self.v_offsets = self.v_template * torch.reshape(scale, [-1, 1, 1]) + displacements
        ...
        results = stacked - \
                  self.pack(
                      torch.matmul(
                          stacked,
                          torch.reshape(
                              torch.cat((J, torch.zeros((batch_num, 24, 1), dtype=torch.float32).to(self.device)),
                                        dim=2),
                              (batch_num, 24, 4, 1)
                          )
                      )
...
def test(gpu_id=[0]):
    ...
    model = SMPLModel(device=device, model_path='../assets/neutral_smpl.pkl')
    upmodel = SMPLModel(device=device, model_path='../assets/upsample_neutral_smpl.pkl') # ../assets/neutral_smpl.pkl

    # for i in range(10):
    pose = np.zeros([32, 24, 3])
    # pose[0][1] = [0, 0, 1.57/2]
    # pose[0][2] = [0, 0, -1.57/2]
    pose = (np.random.rand(32, pose_size) - 0.5) * 0.4 #  pose.reshape([32, -1])
    ...
    # pose = torch.from_numpy((np.zeros([32, pose_size]))) \
    #     .type(torch.float32).to(device)
    # betas = torch.from_numpy((np.zeros([32, beta_size]))) \
    #     .type(torch.float32).to(device)

    offets = torch.from_numpy(np.zeros([32, 6890, 3])).type(torch.float32).to(device)
    upoffets = torch.from_numpy(np.zeros([32, 27554, 3])).type(torch.float32).to(device)
    s = time()
    trans = torch.from_numpy(np.zeros((32, 3))).type(torch.float32).to(device)
    result = model(betas, pose, trans, offets)
    upresult = upmodel(betas, pose, trans, upoffets)
    print(time() - s)

    for i in range(result.shape[0]):
        model.write_obj(result[i], './smpl_torch_{}.obj'.format(i))
        upmodel.write_obj(upresult[i], './upsmpl_torch_{}.obj'.format(i))
